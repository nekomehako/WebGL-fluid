window.onload = function(){
    ...
    const backprgUL = {
        ...
        dt         : gl.getUniformLocation(backprg, 'dt'),
        preColor   : gl.getUniformLocation(backprg, 'preColor'),
        preData    : gl.getUniformLocation(backprg, 'preData'),
    };
    const passprgUL = {
        preColor   : gl.getUniformLocation(passprg, 'preColor'),
        preData    : gl.getUniformLocation(passprg, 'preData'),
    };
    ...
    function loop(){
        ...
        //uniform float time;
        gl.uniform1f(backprgUL['dt'], dt);
        //uniform sampler2D preColor;
        setUniformTexture(backprgUL['preColor'], 0, saveFrameBuffer.t['color']);
        setUniformTexture(backprgUL['preData'],  1, saveFrameBuffer.t['data']);
        ...
        setUniformTexture(passprgUL['preColor'], 0, backFrameBuffer.t['color']);
        gl.drawElements(gl.TRIANGLES, indices.length, gl.UNSIGNED_SHORT, 0);
        ...
        gl.clear(gl.COLOR_BUFFER_BIT);
        setUniformTexture(passprgUL['preColor'], 0, backFrameBuffer.t['color']);
        setUniformTexture(passprgUL['preData'],  1, backFrameBuffer.t['data']);
    ...
    function create_fbo(width, height){
        ...
        // テクスチャオブジェクトの作成
        const colorTexture = create_screenTexture(width, height);
        // テクスチャオブジェクトの関連付け
        gl.bindTexture(gl.TEXTURE_2D, colorTexture);
        // フレームバッファーにテクスチャオブジェクトを入れる
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, colorTexture, 0);

        const dataTexture = create_screenTexture(width, height);
        ...
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
        gl.bindTexture(gl.TEXTURE_2D, null);
        return {f: frameBuffer, t:{color:colorTexture, data:dataTexture}}
    }
